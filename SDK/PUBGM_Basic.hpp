...
struct FString : private TArray<unsigned short>
{
	...
	inline const char* ToString() const
	{
		...
		if (wcstombs(s, ToWString(), Count) > 0)
			s[Count] = '\0';
		return s;
	}
	...
	{
		return memcmp(this->Data, a.Data, this->Num()) == 0;
	};

	inline bool operator==(const char *&a) const
	{
		return !strcmp(this->ToString(), a);
	};

	inline bool operator==(const wchar_t *&a) const
	{
		return !wcscmp(this->ToWString(), a);
	};
};
